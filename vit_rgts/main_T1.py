...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (attn, ff) in enumerate(self.layers):
            ...
            with torch.no_grad():
                token_norms = torch.norm(x.detach(), p=2, dim=-1) + 1e-6                    ##COMPUTES EACH TOKEN NORMS 
                mean_norm = token_norms.mean()                                              ##MEAN AND STD OF NORMS
                std_norm = token_norms.std()
                threshold = mean_norm + (self.N_STD_DEVIATIONS_FOR_THRESHOLD * std_norm)    ## mu + 3 sigma 
...
class VitRGTS(nn.Module):
    ...
    def __init__(
        ...
        *, 
        ...
        heads, 
        mlp_dim,
        num_register_tokens: int = 0,           # no register tokens
        pool = 'cls', 
        channels = 3, 
        dim_head = 64, 
        dropout = 0., 
