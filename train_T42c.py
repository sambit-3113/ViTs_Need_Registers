...
def extract_tokens_patches_labels(model, loader, device, patch_size):
    ...
    return tokens.float(), patches.float(), labels.long()                   ###final-layer patch embeddings, raw pixel patches, class label of parent image 
...
class PatchDecoder(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, hidden=1024):
        ...
        self.net = nn.Sequential(
            nn.Linear(in_dim, hidden),                                   # 1024 -> 512 -> 588 (14 x 14 x 3)
...
def train_decoder(X_tr, Y_tr_patches, X_va, Y_va_patches, in_dim, out_dim, device, epochs=10, bs=2048, lr=1e-3):
    model = PatchDecoder(in_dim, out_dim).to(device)            ###X_tr = patch embedding, Y_tr_patches = flattened patch, X_va, Y_va = for validation
